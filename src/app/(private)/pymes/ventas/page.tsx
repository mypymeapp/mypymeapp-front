...
// TIPO AÑADIDO: Unifica Order con el campo 'total' calculado
interface EnrichedOrder extends Order {
  total: number;
}

// TIPO AÑADIDO: Unión para manejar ambos tipos de registro de forma segura
type SalesRecord = Invoice | EnrichedOrder;
...
export default function ModuloVentas() {
  ...
  const filteredRecords = useMemo(() => {
    // CORRECCIÓN 1: Usar el tipo SalesRecord en lugar de any[]
    ...
    return data.filter(
      (r) =>
        (("customer" in r && r.customer?.name) || ("supplier" in r && r.supplier?.name) || "")
          .toLowerCase()
  ...
  if (isLoading) {
    return (
      <div className="p-8 h-full flex flex-col justify-center items-center">
        ...
        <p className="mt-4 text-foreground/70">Cargando ventas...</p>
      </div>
    );
  }
    ...
    <div className="p-4 md:p-8">
      ...
      {visibleRecords.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {/* CORRECCIÓN 2: Usar el tipo SalesRecord en el map */}
          {visibleRecords.map((r: SalesRecord) => (
            <Card key={r.id} className="flex flex-col">
              <div className="p-4 flex-grow">
                <h2 className="font-bold text-lg text-foreground truncate">
                  {r.number}
                  {'invoiceNumber' in r && r.invoiceNumber ? ` - Factura: ${r.invoiceNumber}` : ""}
                </h2>
                <p className="text-sm text-foreground/70">
                  {'customer' in r ? r.customer?.name : ('supplier' in r ? r.supplier?.name : "Desconocido")}
                </p>
                <p className="text-xs text-foreground/60">
                  {new Date('issuedAt' in r ? r.issuedAt : r.createdAt).toLocaleDateString()}
                </p>
                {activeTab === "orders" && 'items' in r && (
        ...
        <div className="bg-card border border-border rounded-lg overflow-hidden">
          <table className="w-full text-left">
            ...
            <tbody>
              {/* CORRECCIÓN 3: Usar el tipo SalesRecord en el map */}
              {visibleRecords.map((r: SalesRecord) => (
                <tr
                  key={r.id}
                ...
                >
                  <td className="p-4">
                    {r.number}
                    {'invoiceNumber' in r && r.invoiceNumber ? `- ${r.invoiceNumber}` : ""}
                  </td>
                  <td className="p-4">
                    {'customer' in r ? r.customer?.name : ('supplier' in r ? r.supplier?.name : "Desconocido")}
                  </td>
